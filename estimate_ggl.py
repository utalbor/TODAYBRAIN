...
def estimate_ggl(S, A_mask, alpha=0, prob_tol=1e-4, inner_tol=1e-6, max_cycle=20, regularization_type=1):
    ...
    while (not converged) and cycle < max_cycle:
        ...
        for u in range(n):
            ...
            c_uu = C[u, u]
            Ou_i = C[np.ix_(minus_u, minus_u)] - np.matmul(np.expand_dims(c_u, axis=1),
                                                           np.expand_dims(c_u, axis=1).T) / c_uu
            ...
            C[np.ix_(minus_u, minus_u)] = (Ou_i + np.matmul(cu, cu.T) / k_uu)
